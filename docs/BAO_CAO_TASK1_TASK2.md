...
8. [Phụ lục triển khai chi tiết](#8-phụ-lục-triển-khai-chi-tiết-để-đưa-vào-báo-cáo-nộp)
9. [Task 3: Machine Learning & Realtime Dashboard](#9-task-3-machine-learning--realtime-dashboard)
...
## 9. Task 3: Machine Learning & Realtime Dashboard

### 9.1. Mục tiêu Task 3

Task 3 tập trung vào việc xây dựng lớp phân tích thông minh phía trên Gold Layer, gồm 4 phần chính:

1. Xây dựng mô hình học máy cho dự đoán xu hướng và phát hiện bất thường.
2. Tạo pipeline huấn luyện tự động từ dữ liệu Gold (Trino).
3. Xây dựng web UI local để hiển thị kết quả 3 nhóm mô hình.
4. Bổ sung cơ chế realtime (SSE) để dashboard tự cập nhật.

**Kết quả đạt được:**
- Đã hoàn thiện thư mục `ML/` gồm code model, pipeline dữ liệu, script train, web app Flask, JS chart realtime.
- Có thể chạy local tại `http://localhost:5000`.
- Dashboard nhận dữ liệu mới định kỳ và cập nhật tín hiệu model gần realtime.

**Gợi ý ảnh/code nên dán:**
- Ảnh cây thư mục `ML/` trong VS Code Explorer.
- Ảnh chạy app thành công ở `http://localhost:5000`.
- Trích đoạn `ML/README.md` phần kiến trúc và cách chạy.

---

### 9.2. Kiến trúc ML end-to-end

Luồng ML được triển khai như sau:

```
Gold Delta (GCS) → Trino (delta_gcs.gcs.gold_ohlcv)
→ fetch_data.py (feature engineering)
→ train_all.py (train XGBoost + LightGBM + LSTM + Isolation Forest)
→ saved models (ML/models/saved)
→ Flask app.py (inference API + SSE)
→ Dashboard UI (HTML/CSS/JS + Chart.js)
```

Thành phần chính:
- **Data source**: Bảng `gold_ohlcv` qua Trino.
- **ML backend**: Python (scikit-learn, xgboost, lightgbm, tensorflow).
- **Serving**: Flask API + SSE stream (`/stream`).
- **Frontend**: Chart.js realtime charts.

**Gợi ý ảnh/code nên dán:**
- Sơ đồ kiến trúc ML tự vẽ (hoặc copy ASCII ở trên).
- Trích đoạn `ML/data/fetch_data.py` phần query Trino.
- Ảnh response mẫu của endpoint `/api/predictions`.

---

### 9.3. Cấu trúc thư mục ML và vai trò từng file

| Đường dẫn | Vai trò |
|---|---|
| `ML/data/fetch_data.py` | Lấy dữ liệu từ Trino, fallback cache/mock, feature engineering |
| `ML/models/xgboost_lgbm.py` | Train + infer 2 model classification (XGBoost/LightGBM) |
| `ML/models/lstm_model.py` | Train + infer LSTM regression dự đoán giá close tiếp theo |
| `ML/models/isolation_forest.py` | Train + infer phát hiện anomaly |
| `ML/train_all.py` | Script one-click train toàn bộ models |
| `ML/app.py` | Flask app: REST API, SSE, inference orchestration |
| `ML/templates/index.html` | Giao diện dashboard |
| `ML/static/js/app.js` | Gọi API, render chart, kết nối SSE |
| `ML/static/css/style.css` | Styling dashboard |

**Gợi ý ảnh/code nên dán:**
- Ảnh tab file `ML/` đang mở nhiều file model.
- Trích đoạn ngắn đầu mỗi file model để chứng minh phân tách trách nhiệm.

---

### 9.4. Nguồn dữ liệu và Feature Engineering

#### 9.4.1. Nguồn dữ liệu học

- Nguồn chính: `delta_gcs.gcs.gold_ohlcv` (khung 5 phút cho model huấn luyện).
- Fallback:
    - Nếu Trino tạm thời lỗi → đọc `ML/data/cached_ohlcv.csv`.
    - Nếu cache không có → sinh mock data để test nhanh UI/pipeline.

#### 9.4.2. Features đã tạo

Từ OHLCV gốc, pipeline tạo thêm các đặc trưng:
- `price_change_pct`
- `volatility`
- `ma_7`, `ma_20`, `ma_cross_7_20`
- `volume_sma_10`, `volume_spike`
- `rsi_14`
- target classification: `target_bullish`

Ý nghĩa:
- Nhóm feature giá + MA phục vụ dự đoán xu hướng (bull/bear).
- Nhóm biến động + volume phục vụ anomaly detection.
- Chuỗi `close` phục vụ LSTM regression.

**Gợi ý ảnh/code nên dán:**
- Trích đoạn code `feature_engineering()` trong `ML/data/fetch_data.py`.
- Ảnh DataFrame head sau feature engineering (`processed.head()`).
- Ảnh biểu đồ giá + MA trên dashboard.

---

### 9.5. Chi tiết 3 nhóm mô hình ML

> Lưu ý: Phần classification gồm 2 model song song (XGBoost và LightGBM), LSTM là regression, Isolation Forest là anomaly detection.

#### 9.5.1. XGBoost + LightGBM (Classification)

Mục tiêu:
- Dự đoán nến kế tiếp tăng/giảm (`target_bullish`).

Thiết kế:
- Split train/test theo thời gian (không shuffle) để tránh data leakage.
- Đánh giá bằng: Accuracy, Precision, Recall, F1.
- Lưu model `.pkl` và feature importance để hiển thị UI.

Output realtime:
- `xgboost_bull_prob`, `xgboost_signal`
- `lgbm_bull_prob`, `lgbm_signal`

**Gợi ý ảnh/code nên dán:**
- Trích đoạn `train_xgboost()`, `train_lightgbm()`, `predict_latest()`.
- Ảnh bảng metrics trên dashboard (Accuracy/F1).
- Ảnh biểu đồ Feature Importance.

#### 9.5.2. LSTM (Regression)

Mục tiêu:
- Dự đoán giá close nến tiếp theo (continuous value).

Thiết kế:
- Sequence length = 30 nến gần nhất.
- MinMaxScaler cho chuỗi close.
- Mạng 2 lớp LSTM + Dropout, loss = MSE.
- Lưu model `lstm_model.h5` + scaler `lstm_scaler.pkl`.
...
**Gợi ý ảnh/code nên dán:**
- Trích đoạn `prepare_lstm_data()`, `build_and_train_lstm()`, `predict_lstm()`.
- Ảnh chart Actual vs Predicted.
- Ảnh chart Training/Validation Loss.

#### 9.5.3. Isolation Forest (Anomaly Detection)

Mục tiêu:
- Phát hiện các nến bất thường (pump/dump, volume spike, biến động mạnh).

Thiết kế:
- Feature: `price_change_pct`, `volatility`, `volume`.
- `contamination=0.05` (giả định 5% bất thường).
- Output gồm nhãn anomaly và score.
...
**Gợi ý ảnh/code nên dán:**
- Trích đoạn `train_isolation_forest()` và `detect_anomalies()`.
- Ảnh anomaly timeline chart.
- Ảnh bảng anomalies gần nhất trên dashboard.

---

### 9.6. Pipeline huấn luyện và artifacts

Script `ML/train_all.py` thực hiện one-click training:

1. Fetch dữ liệu từ Trino.
2. Feature engineering.
3. Train XGBoost.
4. Train LightGBM.
5. Train LSTM.
6. Train Isolation Forest.
7. Ghi artifacts và metrics.
...
- `xgb_feature_importance.csv`, `lgbm_feature_importance.csv`
- `lstm_history.csv`

**Gợi ý ảnh/code nên dán:**
- Ảnh terminal chạy `python ML/train_all.py` (đoạn summary metrics cuối).
- Ảnh thư mục artifacts sau khi train.
- Trích đoạn code save model/metrics.

---

### 9.7. Web UI local và cơ chế realtime

#### 9.7.1. Web app Flask

App `ML/app.py` cung cấp:
- Trang UI: `/`
- API inference + data:
    - `/api/predictions`
    - `/api/training-results`
    - `/api/feature-importance`
    - `/api/lstm-predictions`
    - `/api/lstm-history`
    - `/api/price-history`
    - `/api/anomalies`
- SSE stream: `/stream`

#### 9.7.2. Realtime update

- Background worker chạy chu kỳ 30 giây:
    1. Pull dữ liệu mới từ Trino.
    2. Feature engineering.
    3. Inference cả 3 nhóm model.
    4. Cập nhật cache `latest_predictions`.
- Frontend dùng `EventSource('/stream')` để nhận update realtime.

#### 9.7.3. Đồng bộ Gold cho ML

- Mỗi 5 phút, app trigger `silver_to_gold.py` qua `docker exec spark-master spark-submit`.
- Mục tiêu: đảm bảo lớp Gold luôn mới để inference không bị stale.

**Gợi ý ảnh/code nên dán:**
- Trích đoạn route `/stream` trong `ML/app.py`.
- Trích đoạn JS `connectSSE()` trong `ML/static/js/app.js`.
- Ảnh dashboard đang auto cập nhật (giờ cập nhật thay đổi theo thời gian).
- Ảnh log backend thể hiện chu kỳ `[SSE] Updated...`.

---

### 9.8. Các bước chạy Task 3 từ đầu đến cuối

1. Tạo và activate môi trường Python cho ML.
2. Cài dependencies từ `ML/requirements.txt`.
3. Đảm bảo Trino + Spark + Airflow stack đang chạy.
4. Train models bằng `python ML/train_all.py`.
5. Chạy dashboard bằng `python ML/app.py`.
6. Mở `http://localhost:5000` và kiểm tra các panel.

Checklist pass:
- Dashboard vào được trang chủ.
- Các API trả HTTP 200.
- Có dữ liệu prediction cho XGB/LGBM/LSTM.
- Có anomaly data và chart hiển thị.
- SSE nhận update định kỳ.

**Gợi ý ảnh/code nên dán:**
- Ảnh terminal từng bước setup/run.
- Ảnh test endpoint trả 200.
- Ảnh đầy đủ 4 panel dashboard trên cùng 1 màn hình.

---

### 9.9. Kết quả đạt được của Task 3

| Hạng mục | Kết quả |
|---|---|
| Code 3 nhóm model | ✅ Hoàn thiện (XGBoost/LightGBM, LSTM, Isolation Forest) |
| Data pipeline cho model | ✅ Có fetch Trino + feature engineering + cache/mock fallback |
| UI local hiển thị kết quả model | ✅ Flask + Chart.js dashboard |
| Realtime update | ✅ SSE stream + background worker |
| Khả năng chạy local end-to-end | ✅ Có thể train + serve + visualize trên máy local |

**Gợi ý ảnh/code nên dán:**
- Ảnh bảng kết quả tổng hợp này sau khi điền số liệu thực tế của nhóm.
- Ảnh metrics từ `training_results.json` hoặc dashboard.

---

### 9.10. Hạn chế hiện tại và hướng cải tiến

Hạn chế:
1. Dữ liệu train hiện ưu tiên 1 symbol (BTCUSDT) cho demo nhanh.
2. Chưa có model registry/versioning chuẩn (MLflow, DVC, etc.).
3. Chưa có online learning thực thụ; hiện là retrain theo batch.
4. Chưa có threshold tuning tự động cho anomaly theo thị trường từng giai đoạn.

Hướng cải tiến:
1. Train multi-symbol + multi-timeframe và ensemble voting.
2. Thêm backtesting module (PnL, Sharpe, max drawdown).
3. Đưa inference lên service riêng (FastAPI + worker queue).
4. Tích hợp alert realtime (Telegram/Slack) khi anomaly lớn.

**Gợi ý ảnh/code nên dán:**
- Trích đoạn TODO roadmap ngắn (nếu có file kế hoạch nhóm).
- Ảnh minh hoạ alert mock hoặc biểu đồ backtest (nếu triển khai thêm).

---

### 9.11. Checklist minh chứng riêng cho Task 3 (nên chụp)

| # | Minh chứng bắt buộc | Nên chụp gì |
|---|---|---|
| 1 | Cấu trúc ML | Explorer mở `ML/` với đầy đủ file |
| 2 | Train thành công | Terminal chạy `train_all.py` + metrics summary |
| 3 | Model artifacts | Thư mục `ML/models/saved/` sau train |
| 4 | UI chạy local | Màn hình dashboard `http://localhost:5000` |
| 5 | Realtime hoạt động | Video ngắn/2 ảnh trước-sau timestamp update |
| 6 | API hoạt động | Response JSON từ `/api/predictions` |
| 7 | Feature importance | Chart XGB/LGBM |
| 8 | LSTM chất lượng | Chart Actual vs Predicted + Loss |
| 9 | Anomaly detection | Bảng anomalies + score timeline |
| 10 | Tích hợp lakehouse | Log refresh Gold hoặc query Trino mới nhất |
