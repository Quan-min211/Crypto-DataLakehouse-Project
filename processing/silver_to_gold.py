...
def build_ohlcv_candles(df: DataFrame, window_duration: str) -> DataFrame:
    ...
    # FIX: F.first()/F.last() in groupBy().agg() are NON-DETERMINISTIC in Spark.
    # Spark processes data in parallel — row order within a partition after groupBy
    # is NOT guaranteed. This means open/close could be WRONG on every run.
    #
    # Solution: Use F.min/max(struct(event_time, price)) trick.
    # F.min(struct(a, b)) returns the struct where 'a' is minimum (lexicographic).
    # So min(struct(event_time, price)).price = price at EARLIEST event_time = OPEN.
    # And max(struct(event_time, price)).price = price at LATEST event_time = CLOSE.
    ohlcv = (
        windowed
        .groupBy("symbol", "candle_time")
        .agg(
            F.min(F.struct("event_time", "price")).getField("price").alias("open"),
            F.max("price").alias("high"),
            F.min("price").alias("low"),
            F.max(F.struct("event_time", "price")).getField("price").alias("close"),
