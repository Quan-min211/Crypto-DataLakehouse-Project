...
function apiUrl(path) {
    ...
    return `${path}${sep}symbol=${encodeURIComponent(selectedSymbol)}`;
}
...
function updatePredictionUI(data) {
    if (data.symbol) {
        document.getElementById('tickerSymbol').textContent = data.symbol;
    }
...
async function renderLSTMChart() {
    try {
        const res = await fetch(apiUrl('/api/lstm-predictions'));
        const data = await res.json();
...
async function renderPriceChart() {
    try {
        const res = await fetch(apiUrl('/api/price-history'));
        const data = await res.json();
...
function connectSSE() {
    ...
    sseConnection = new EventSource(apiUrl('/stream'));
    sseConnection.onmessage = function (event) {
        try {
            const data = JSON.parse(event.data);
            updatePredictionUI(data);
            console.log(`[SSE] Update received (${selectedSymbol}):`, data.timestamp);
        } catch (e) {
...
async function reloadAllForSymbol() {
    await Promise.all([
        fetchPredictions(),
        renderLSTMChart(),
        renderAnomalies(),
...
function setupSymbolSelector() {
    ...
    selectedSymbol = selector.value || selectedSymbol;
    document.getElementById('tickerSymbol').textContent = selectedSymbol;

    selector.addEventListener('change', async (e) => {
        selectedSymbol = e.target.value;
        document.getElementById('tickerSymbol').textContent = selectedSymbol;
        await reloadAllForSymbol();
        connectSSE();
    });
}
