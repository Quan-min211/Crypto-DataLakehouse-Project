...
cached_data_by_symbol = {}          # {symbol: DataFrame đã feature engineering}
latest_predictions_by_symbol = {}   # {symbol: latest predictions}
...
def normalize_symbol(symbol: str | None) -> str:
    """Chuẩn hóa symbol và fallback về default nếu symbol không hợp lệ."""
    if not symbol:
        return DEFAULT_SYMBOL
    symbol = symbol.strip().upper()
    return symbol if symbol in SUPPORTED_SYMBOLS else DEFAULT_SYMBOL


def get_symbol_from_request() -> str:
    """Lấy symbol từ query params: ?symbol=BTCUSDT"""
    return normalize_symbol(request.args.get("symbol"))


def get_cached_data(symbol=DEFAULT_SYMBOL, force_refresh=False):
    """Lấy hoặc refresh cached data"""
    symbol = normalize_symbol(symbol)
    df_cached = cached_data_by_symbol.get(symbol)

    if force_refresh or df_cached is None or len(df_cached) == 0:
        try:
            raw = fetch_data_from_trino(symbol, limit=5000)
            cached_data_by_symbol[symbol] = feature_engineering(raw)
        except Exception as e:
            log.warning(f"Fetch thất bại ({symbol}), dùng mock: {e}")
            raw = generate_mock_data(symbol, 500)
            cached_data_by_symbol[symbol] = feature_engineering(raw)

    return cached_data_by_symbol.get(symbol)
...
def sse_worker():
    ...
    while True:
        try:
            ...
            # Fetch data + chạy predictions cho danh sách symbol hỗ trợ
            for symbol in SUPPORTED_SYMBOLS:
                df = get_cached_data(symbol, force_refresh=True)
                if df is None or len(df) == 0:
                    continue

                latest_predictions_by_symbol[symbol] = run_predictions(df)
                latest = latest_predictions_by_symbol[symbol]
                log.info(
                    f"[SSE] Updated({symbol}): price={latest.get('current_price')}, "
                    f"xgb={latest.get('xgboost_signal')}, "
                    f"lstm={latest.get('lstm_predicted_price')}"
                )
...
def api_predictions():
    ...
    symbol = get_symbol_from_request()

    if symbol not in latest_predictions_by_symbol:
        df = get_cached_data(symbol)
        if df is None or len(df) == 0:
            return jsonify({"error": f"No data for {symbol}"}), 404
        latest_predictions_by_symbol[symbol] = run_predictions(df)

    return jsonify(latest_predictions_by_symbol[symbol])
...
def stream():
    ...
    def event_stream():
        while True:
            ...
            latest = latest_predictions_by_symbol.get(symbol)
            if latest:
